...
class CherryPickStudy(
    ...
    init {
        ...
        val repoPath: Path = org.anon.evaluation.cloneGitHubRepo(config, dataset.repositoryId)
        ...
        for (i in 1..numThreads) {
            ...
            org.anon.evaluation.cleanVariantDirectories(operations)
            // Copy the source and target variant to the respective variant directories
            org.anon.evaluation.prepareVariantDirectories(operations, repoPath)
...
fun main(args: Array<String>) {
    ...
    val allSamples = org.anon.evaluation.createOrLoadSamples(config, datasetsPerLanguage, rand)

    org.anon.evaluation.cloneDatasets(allSamples, config)

    val n = 10
    Logger.info("Processing $n repos in parallel")
    val threadPool = Executors.newFixedThreadPool(n)

    val completedRunsAll = org.anon.evaluation.loadCompletedRuns(config)
    ...
    for (repetition in config.EXPERIMENT_REPEATS_START()..config.EXPERIMENT_REPEATS_END()) {
        ...
        val numCherryPicks = org.anon.evaluation.countCherryPicks(allSamples[repetitionIndex])
        ...
        for (dataset in allSamples[repetitionIndex]) {
            ...
            if (completedRuns.contains(dataset.datasetName) && completedRuns[dataset.datasetName]!!.size == dataset.cherryPicks.size) {
                ...
                org.anon.evaluation.printProgress(completed, numCherryPicks, repetition, 0uL)
                continue
            }
            threadPool.submit {
                val i = id
                Logger.info("Preparing evaluation of cherry picks from ${dataset.datasetName}")
                val study = org.anon.evaluation.CherryPickStudy(
                    config,
                    dataset,
                    repetition,
                    idProvider,
                ...
                org.anon.evaluation.printProgress(completed, numCherryPicks, repetition, i)
            }
        }
...
private fun cloneDatasets(
    ...
    for (dataset in datasetsToClone) {
        threadPool.submit {
            try {
                org.anon.evaluation.cloneGitHubRepo(config, dataset.repositoryId)
            } catch (e: Exception) {
                Thread.sleep(60_000)
                org.anon.evaluation.cloneGitHubRepo(config, dataset.repositoryId)
            }
...
private fun createOrLoadSamples(
    ...
    for (language in langs) {
        ...
        val sample: List<List<CherryDataset>> = if (config.EXPERIMENT_ENABLE_SAMPLING()) {
            ...
            org.anon.evaluation.sampleCherries(config, datasets, rand)
        } else {
...
fun sampleCherries(config: EvalConfig, datasets: List<CherryDataset>, rand: SecureRandom): List<List<CherryDataset>> {
    ...
    for (repetition in config.EXPERIMENT_REPEATS_START()..config.EXPERIMENT_REPEATS_END()) {
        ...
        val sampledCherries = org.anon.evaluation.countCherryPicks(datasetSubset)
...
fun loadPRDatasets(config: EvalConfig): Map<String, MutableList<CherryDataset>> {
    ...
    for (yamlFile in org.anon.evaluation.getYamlFiles(config.EXPERIMENT_DATASETS())) {
        val dataset = org.anon.evaluation.loadDataset(yamlFile, config.EXPERIMENT_CHERRY_TYPE())
...
fun getYamlFiles(directoryPath: Path): List<Path> {
    val yamlFileVisitor = org.anon.evaluation.YamlFileVisitor()
...
fun loadDataset(pathToYaml: Path, cherryType: org.anon.evaluation.CherryType): Optional<CherryDataset> {
    ...
    for (cp in prEntries) {
        ...
        if (cherryType == org.anon.evaluation.CherryType.Trivial && !isTrivial) {
            continue
        } else if (cherryType == org.anon.evaluation.CherryType.Complex && isTrivial) {
            continue
        }
...
private fun prepareVariantDirectories(operations: EvalOperations, gitHubRepoPath: Path) {
    ...
    operations.shell.execute(
        CpCommand(
            gitHubRepoPath,
            operations.targetVariantV0
        ).recursive())
        .expect("Was not able to copy target variant V0.")
    operations.shell.execute(
        CpCommand(
            gitHubRepoPath,
            operations.targetVariantV1
        ).recursive())
