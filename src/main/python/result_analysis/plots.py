...
from result_analysis.result_handling import non_trivial_results, overall_automation
...
def ed_runtime_boxplot_results_lang_overall(
    ...
    for patcher in Patcher:  # Patcher is an enum
        ...
        edit_distance_per_language = []
        runtime_per_language = []
        ...
        for language in languages:
            ...
            for res in results:
                precision, recall = calculate_precision_recall(
                    res.outcome_classification.tp(),
                    res.outcome_classification.fp(),
                    res.outcome_classification.fn(),
                )
...
def boxplot_results_lang_per_proj(
    ...
    for patcher in Patcher:  # Patcher is an enum
        ...
        results = results_per_repo(results, repos)
        # Accumulate repo results per language
        lang_results = project_results_per_language(results, min_results_per_repo)
        precision_per_language = []
        recall_per_language = []
        for language in languages:
            language = language[0]
...
def automation_boxplot_results_lang_per_proj(
    ...
    for patcher in Patcher:  # Patcher is an enum
        ...
        patchers.append(patcher)
        automation_per_patcher.append(automation_per_language)
...
def boxplot_results_per_patcher(
    path_to_results, path_to_repo_list, min_results_per_repo
):
    repos = load_repositories(path_to_repo_list)
    precisions_per_patcher = []
    recalls_per_patcher = []
