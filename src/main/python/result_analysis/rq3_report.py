...
min_denominator = (
    5  # projects with: "every many-th commit is a cherrypick or more" are noteworthy
)
...
def read_cherry(f):
    with open(f, "r", encoding="utf-8") as file:
        ...
        project_info[c_language], project_info[c_commits], project_info[c_cherries] = (
            project_info[c_language],
            int(project_info[c_commits]),
            int(project_info[c_cherries]),
        )
        return project_info
...
def read_yamls(files):
    ...
    for i, repo in enumerate(repo_sample_info):
        row_num = pr_df[pr_df[c_repo_name] == repo["full_name"]].index.values.astype(
            int
        )[0]
...
def column_report(lang, column_description, column, df):
    ...
    print(
        f'{lang}, for column "{column_description}" the quantiles ({quantiles} chosen) are: {formatted_list}'
    )
...
def get_siunit_column_format(df):
    ...
    for c in df:
        if pd.api.types.is_numeric_dtype(df[c]):
            ...
            column_header += (
                f"S[table-format={before_dot}.{after_dot}, round-precision={after_dot}]"
            )
        else:
            column_header += "l"
...
def df_to_latex(pr_df):
    ...
    df[c_projects_with_cherries] = (
        pr_df[pr_df[c_cherries] > 0].groupby(c_language)[c_cherries].count().values
    )

    c_pick_list = [
        c_language,
        c_sampled_projects_per_language,
        c_projects_with_cherries,
        c_cherries,
        c_cherry_ratio,
        c_trivial_cherries,
    ]
    ...
    df.loc[c_total] = [c_total] + [
        df[p].sum() for p in c_pick_list if pd.api.types.is_numeric_dtype(df[p])
    ]
    df.loc[c_total, c_cherry_ratio] = (
        pr_df[c_cherries].sum() / pr_df[c_commits].sum() * 100
    )
    df.loc[c_total, c_trivial_cherries] = (
    ...
    to_latex(
        df,
        label="tab:overall",
        caption="Overview of our collection of GitHub projects.",
        position=table_pos,
        column_format=get_siunit_column_format(df),
        addmr=True,
    )
    pass
...
def report_projects(pr_df):
    ...
    if len(pr_df[c_language].unique()) > 1:
        extreme_projects = pr_df[pr_df[c_cherry_ratio] > min_ratio].sort_values(
            by=c_cherry_ratio
        )
        print(
            f"Most extreme projects, with more than 1 in {min_denominator} (>{min_ratio}) commits being a cherry-pick:"
        )
    ...
    print(
        f"Project# distribution by language: {[(l, len(df[df[c_language] == l])) for l in langs]}"
    )
...
def setup_plt():
    path_font = (
        "C:\\Users\\lanpi\\AppData\\Local\\Microsoft\\Windows\\Fonts\\cmunrm.ttf"
    )
...
def correlate(df):
    ...
    plt.xticks(
        range(corr.select_dtypes(["number"]).shape[1]),
...
def impact_file(file):
    ...
    with open(file, "r", encoding="utf-8") as file:
        ...
        for l in lines:
            ...
            nR = di["normalResult"]
            if (
                nR["missing"] + nR["applied"] + nR["wrongLocation"] == 0
            ):  # skip if nothing should have been done
                continue
            seen += 1
            rq += (
                nR["wrongLocation"] * 2 + nR["missing"] + nR["invalid"]
            )  # required fixes + FP + FN
            if (
                nR["wrongLocation"] + nR["missing"] + nR["invalid"] == 0
            ):  # nothing went wrong?, then it is automatically executable
                ap += 1
...
def impact_analysis(pr_df):
    ...
    idf = pd.DataFrame(
        columns=[c_repo_name]
        + ["rq_" + app for app in approaches]
        + ["ap_" + app for app in approaches]
    )
    ...
    for index, row in idf.iterrows():
        idf.loc[index, c_language] = pr_df[pr_df[c_repo_name] == row[c_repo_name]][
            c_language
        ].values[0]
        idf.loc[index, c_cherries] = pr_df[pr_df[c_repo_name] == row[c_repo_name]][
            c_cherries
        ].values[0]
        idf.loc[index, c_cherry_ratio] = (
            pr_df[pr_df[c_repo_name] == row[c_repo_name]][c_cherries].values[0]
            / pr_df[pr_df[c_repo_name] == row[c_repo_name]][c_commits].values[0]
        )
        idf.loc[index, c_trivial_cherries] = (
            pr_df[pr_df[c_repo_name] == row[c_repo_name]][c_cherries].values[0]
            - pr_df[pr_df[c_repo_name] == row[c_repo_name]][c_trivial_cherries].values[
                0
            ]
        )
